...
func AddMaterialInfo(c *gin.Context) {
	...
	if err != nil {
		...
		log.Warnf(context.Background(), "TransID:%s,新建物料信息请求参数无效:%v", transID, err)
		return
	}
...
func UpdateMaterialInfo(c *gin.Context) {
	...
	if err != nil {
		...
		resp.Message = err.Error()
		c.JSON(http.StatusOK, resp)
		log.Warnf(context.Background(), "TransID:%s,更新物料信息请求参数无效:%v", transID, err)
		return
	}
	err = middleware.Validate.Struct(req)
	if err != nil {
...
func DeleteMaterialInfo(c *gin.Context) {
	...
	if err != nil {
		...
		resp.Message = err.Error()
		c.JSON(http.StatusOK, resp)
		log.Warnf(context.Background(), "TransID:%s,删除物料信息请求参数无效:%v", transID, err)
		return
	}
	err = middleware.Validate.Struct(req)
	if err != nil {
