...
type ProductOrder struct {
	ModelID
	ProductOrderNo              string                    `json:"productOrderNo" gorm:"size:100;comment:生产工单号"`             //生产工单号
	SalesOrderNo                string                    `json:"salesOrderNo" gorm:"size:100;comment:销售单号"`                //销售单号
	ItemNo                      string                    `json:"itemNo" gorm:"size:100;comment:销售项号"`                      //销售项号
	CustomerName                string                    `json:"customerName" gorm:"size:400;comment:客户名称"`                //客户名称
	OrderType                   int32                     `json:"orderType" gorm:"comment:工单类型"`                            //工单类型
	OrderTime                   sql.NullTime              `json:"orderTime" gorm:"comment:下单时间"`                            //下单时间
	OrderQTY                    int32                     `json:"orderQTY" gorm:"comment:下单数量"`                             //下单数量
	Warehouse                   string                    `json:"warehouse" gorm:"size:100;comment:成品仓库"`                   //成品仓库
	ReceiptNoteNo               string                    `json:"receiptNoteNo" gorm:"size:100;comment:成品入库单"`              //成品入库单
	ProductionTeam              string                    `json:"productionTeam" gorm:"size:100;comment:生产班组"`              //生产班组
	PriorityLevel               int32                     `json:"priorityLevel" gorm:"comment:生产优先级"`                       //生产优先级
	DeliveryDate                sql.NullTime              `json:"deliveryDate" gorm:"comment:交货期限"`                         //交货期限
	...
	ReleaseTime                 sql.NullTime              `json:"releaseTime" gorm:"comment:发放时间"`                          //发放时间
	SelectedOptions             string                    `json:"selectedOptions" gorm:"size:2000;comment:选配信息"`            //选配信息
	PropertyBrief               string                    `json:"propertyBrief" gorm:"size:4000;comment:特性简述"`              //特性简述
	StandardWorkTime            int32                     `json:"standardWorkTime" gorm:"comment:标准节拍"`                     //标准节拍
	IssuedQTY                   int32                     `json:"issuedQTY" gorm:"comment:已发料数量"`                           //已发料数量
	LastIssueTime               sql.NullTime              `json:"lastIssueTime" gorm:"comment:最新发料时间"`                      //最新发料时间
	EstimateStartTime           sql.NullTime              `json:"estimateStartTime" gorm:"comment:预计开工时间"`                  //预计开工时间
	ActualStartTime             sql.NullTime              `json:"actualStartTime" gorm:"comment:实际开工时间"`                    //实际开工时间
	StartedQTY                  int32                     `json:"startedQTY" gorm:"comment:已开工数量"`                          //已开工数量
	ObsoletedQTY                int32                     `json:"obsoletedQTY" gorm:"comment:已淘汰数量"`                        //已淘汰数量
	FinishedQTY                 int32                     `json:"finishedQTY" gorm:"comment:已完工数量"`                         //已完工数量
	EstimateConsumeTimePerPiece int32                     `json:"estimateConsumeTimePerPiece" gorm:"comment:预计单件耗时"`        //预计单件耗时
	EstimateFinishTime          sql.NullTime              `json:"estimateFinishTime" gorm:"comment:预计完工时间"`                 //预计完工时间
	ActualFinishTime            sql.NullTime              `json:"actualFinishTime" gorm:"comment:实际完工时间"`                   //实际完工时间
	AverageConsumeTimePerPiece  int32                     `json:"averageConsumeTimePerPiece" gorm:"comment:平均单件耗时"`         //平均单件耗时
	DepositedQTY                int32                     `json:"depositedQTY" gorm:"comment:已入库数量"`                        //已入库数量
	LastDepositTime             sql.NullTime              `json:"lastDepositTime" gorm:"comment:最新入库时间"`                    //最新入库时间
	CurrentState                string                    `json:"currentState" gorm:"size:100;comment:当前状态"`                //当前状态
	LastUpdateTime              time.Time                 `json:"lastUpdateTime" gorm:"autoUpdateTime:nano;comment:最新更新时间"` //最新更新时间
	Remark                      string                    `json:"remark" gorm:"size:1000;comment:备注"`                       //备注
	ProductModelID              *string                   `json:"productModelID" gorm:"size:36;comment:产品型号ID"`             //产品型号ID
	ProductModel                *ProductModel             `json:"productModel" gorm:"constraint:OnDelete:SET NULL"`         //产品型号
	ProductionLineID            *string                   `json:"productionLineID" gorm:"size:36;comment:发放产线ID"`           //发放产线ID
