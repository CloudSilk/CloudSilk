...
# 十、全功能可用性加固（2026-08-15）

针对"功能粗糙不可用"的评审意见做系统性加固（前端 /aps、/monitoring 重写见 8f91a59；后端六模块见 0378d66）。

| 模块 | 加固内容 |
|------|---------|
| 数据完整性 | 库存 (物料,仓库) 唯一索引 + 并发建账 OnConflict 兜底；点位 (设备,名) 唯一；设备删除补维保历史引用检查 |
| SCADA | 采集器热加载（30s 监督周期，设备/点位变更免重启生效）；点位协议校验修复（原 OPC UA 节点必填校验因未预载设备恒不生效）+ Modbus 功能码校验 |
| 质量管理 | /start 开始检验；complete 草稿模式（saveOnly 分批填报）；逐项抽检判定（bySample 按该项标准 AQL，替代整单取首标准简化） |
| WMS | 收货上架库位（料架链跨仓校验）；盘点实盘<锁定量拒绝 |
| 设备管理 | 维保周期变更重算下次执行；周期非法拒绝 |
| APS | 下发前非空校验 |

测试：新增 6 用例，全仓 7 包全绿；Swagger 重生成（712 路径含 /start）；单进程冒烟通过（overview 200、未授权写被 RBAC 拦截）。
存量库注意：AutoMigrate 建库存唯一索引前需先合并 (物料,仓库) 重复账。
